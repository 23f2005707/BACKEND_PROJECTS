...
const Player = () => {
    ...
    useEffect(() => {
        ...
        if (audioRef.current) {
            ...
            ///////// Auto-play new song when selected from sidebar//////
            const playPromise = audioRef.current.play()
            if(playPromise !== undefined) {
                playPromise.catch(() => {}) // avoid browser autoplay error
            }
            setIsPlaying(true)
            // audioRef.current.play()
            // setCurrentTime(0)
    ...
    const togglePlay = () => {
        ...
        if (isPlaying) {
            ...
            audio.play().catch(() => {}) // avoid browser autoplay error
        }
        setIsPlaying(!isPlaying)
    }
    ...
    // safety check 
    if (!song?.url || !song) return <p style = {{textAlign: "center"}}>Select a song</p>
